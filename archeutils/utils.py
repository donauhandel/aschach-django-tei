...
def serialize_project():
    ...
    g.add((prauscher, acdh_ns.hasTitle, Literal("Peter Rauscher", lang=ARCHE_LANG)))
    g.add((prauscher, acdh_ns.hasFirstName, Literal("Peter", lang=ARCHE_LANG)))
    g.add((prauscher, acdh_ns.hasLastName, Literal("Rauscher", lang=ARCHE_LANG)))
    g.add((prauscher, RDF.type, acdh_ns.Person))
    g.add((aserles, acdh_ns.hasTitle, Literal("Andrea Serles", lang=ARCHE_LANG)))
    g.add((aserles, acdh_ns.hasFirstName, Literal("Andrea", lang=ARCHE_LANG)))
    g.add((aserles, acdh_ns.hasLastName, Literal("Serles", lang=ARCHE_LANG)))
    g.add((aserles, RDF.type, acdh_ns.Person))
    g.add((bpamperl, acdh_ns.hasTitle, Literal("Beate Pamperl", lang=ARCHE_LANG)))
    g.add((bpamperl, acdh_ns.hasFirstName, Literal("Beate", lang=ARCHE_LANG)))
    g.add((bpamperl, acdh_ns.hasLastName, Literal("Pamperl", lang=ARCHE_LANG)))
...
def as_arche_graph(res):
    ...
    g.add(
        (
            sub,
            acdh_ns.hasTitle,
            Literal(f"Angabe vom {res.date_german()}  ({res})", lang=ARCHE_LANG),
        )
    )
    ...
    col.add((col_sub, acdh_ns.isPartOf, URIRef(f"{ARCHE_BASE_URL}/aschach")))
    col.add(
        (
            col_sub,
            acdh_ns.hasNonLinkedIdentifier,
    ...
    if res.datum is not None:
        ...
        g.add((sub, acdh_ns.hasCoverageEndDate, Literal(res.datum, datatype=XSD.date)))
    for x in res.get_waren_einheiten["waren"]:
        g.add((sub, acdh_ns.hasSubject, Literal(x.name, lang=ARCHE_LANG)))
    g.add(
        (
            sub,
            acdh_ns.hasCategory,
